...
class Input(object):
	...
	def analyze(self):
		...
		for key in nets:
			if nets[key]:
				if found:
					raise StandardError('analyze error: Multiple true nets', nets, text)
				else:
					self.result = key
					found = True
		if not found:
		...
		for line in self.text:
			for char in line:
				if char == '.':
					...
					raise ValueError("_analyze_text error: invalid character", c, self.filename, self.text)
		...
		with open(filename) as file:
			for line in file:
				...
				if len(line) == 7:
					...
					raise ValueError('read_file error: Invalid input line length.', filename, num_lines, line)
		if num_lines == 9:
			return new_input
		else:
			raise ValueError('read_file error: Invald number of lines', filename, num_lines)
